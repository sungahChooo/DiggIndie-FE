...
export default function CalendarHeader({ onToggleCalendar, isCalendarOpen }: Props) {
  ...
  return (
    <div className="flex w-full h-[56px] items-center bg-black py-4 px-5 justify-between ">
      ...
      <span className="text-[16px] font-semibold text-white">전체 캘린더</span>

      <button onClick={onToggleCalendar} className={'cursor-pointer'}>
        <Image
          src={isCalendarOpen ? redCalendarIcon : calendarIcon}
          alt="calendar"
          width={24}
          height={24}
        />
